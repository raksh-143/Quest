<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if($pword != $cpword){
            ...
            $stmt = $dbh->prepare("update teacher set pword=:pword where id=:tid");
            $stmt->execute([':pword'=>$pword,':tid'=>$id]);
...
<body class="text-center" style="background-color: #F0F3BD;">
    ...
    <form action="teacherPword.php" method="POST">
        <input id="first" class="m-3 p-3 input form-control-lg border-1" type="text" placeholder="Enter Teacher Login ID" required name="id"><br>
